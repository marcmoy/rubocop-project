...
*TODO:* Figure out how to configure cop to ignore comment lines
...
#### Observations:
The current `ThoughBot` settings yields `3653` offenses. When turning off the `EnforcedStyle: double_quotes` in the existing `ThoughBot` settings, this yielded `25080` offenses. This cop asks for either **single or double quotes?**.
...
## Style/Tab
#### Observations:
This cop detects if tabs are detected and gives a warning. When tested against the curriculum, `22` files were affected.

Some personal machines that don't use Atom cannot automatically format tabs into two spaces.
